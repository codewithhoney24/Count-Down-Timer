...
console.log(
  chalk.italic.bold.bgCyanBright(
...
const result = await inquirer.prompt({
  name: "UserInput",
  type: "number",
  message: chalk.italic.bold.bgYellowBright(
...
function startTimer(duration: number) {
  ...
  const interval = setInterval(() => {
    if (timeLeft <= 0) {
      console.log(
        chalk.bold.bgGreenBright(" Successfully😍 ...Countdown completed! ")
      );
    ...
    console.log(
      chalk.bold.blue(
        `Time left : ${min.toString().padStart(2, "0")}:${sec
          .toString()
